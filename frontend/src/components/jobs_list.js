...
function JobsList({
    fetchingFailed, isFetechingJobs, currentJob, jobs, currentPage, selectCurrentJob, fetchPage
  }) {
  return (
              ...
              {jobs.map((job) => {
                  ...
                  <ListItem classes={{root: `job-list-item ${rootClass}`}} key={job.id}
                      ...
                      <div className="location-cont">
                        ...
                        <div className="time">{formatDateTime(job.created_at)}</div>
                      </div>
                  </ListItem>
                  )
              })
        ...
        <Grid item xs={7} classes={{root: 'job-description'}}>
          <div className="job-header">
              {currentJob.company_logo &&
                <img src={currentJob.company_logo} key={currentJob.company_logo} alt=""/>
              }
